...
public:
    AttrMap() { }
    AttrMap( const QString& name, const QVariant& value ) {
		insert( name, value );
    }
	iterator insert( const QString& key, const QVariant& value, bool overwrite = TRUE ) {
		return QMap<QString, QString>::insert(key, value.toString(), overwrite);
	}
};
...
public:
	...
	void writeString( const QVariant& value );
	...
	void writeTaggedString( const QString& name, const QVariant& value,
