    ...
    : indentSize( 4 ), autoNewLine( false ), atBeginningOfLine( true )
{
    ...
    if ( codec == 0 ) {
		...
		out.setCodec( codec );
		out << "<?xml version=\"1.0\" encoding=\"" << protect( codec->mimeName() ) << "\"?>\n";
    }
}
...
void XmlWriter::writeString( const QVariant& value )
{
	out << protect( value.toString() );
    atBeginningOfLine = false;
}
...
void XmlWriter::writeTaggedString( const QString& name, const QVariant& value,
				   const AttrMap& attrs )
{
    writePendingIndent();
    out << opening( name, attrs );
